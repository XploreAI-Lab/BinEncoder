...
def collect_block_xrefs(out, mlist, blk, ins, find_uses):#遍历基本块中的指令，指令之间为双向链表
    p = ins
    breakpoint()
    while p and not mlist.empty():
        ...
        if mlist.has_common(plst):#有相同的寄存器，说明有def-use情况
            if not p.ea in out:
                out.append(p.ea) # this microinstruction seems to use out operand
        mlist.sub(_def)
...
if ida_hexrays.init_hexrays_plugin():#插件可用
    ea = ida_kernwin.get_screen_ea()#获取当前选中指令
    pfn = ida_funcs.get_func(ea)#所在函数
    
    
    w = ida_kernwin.warning
    if pfn:
        F = ida_bytes.get_flags(ea) #获取函数的标志位信息
        if ida_bytes.is_code(F): #当前字节被标记为代码
            ...
            if ida_hexrays.get_current_operand(gco):
                ...
                if mba:
                    ...
                    if merr == ida_hexrays.MERR_OK:
                        ...
                        if gco.append_to_list(mlist, mba): #把mba中的microcode转为mlist（后续要求mlist_t）
                            ctx = ida_hexrays.op_parent_info_t()
                            breakpoint()
                            mop = mba.find_mop(ctx, ea, gco.is_def(), mlist)#找到 mlist_t 中寄存器对应的 mop_t 以及上下文信息(ctx,context)
                            if mop:
