...
def get_xref_func_name(sea):
    ...
    return ['<self>' if f == fname else f for f in list(xref_set_from)], ['<self>' if f == fname else f for f in
                                                                          list(xref_set_to)]
...
def moprand_process(mop):
    ...
    if mop.t == 0:  # mop_z , none
        ...
        r = subins.r
        d = subins.d
        subinst_mc_info = [sub_opcode, moprand_process(l), moprand_process(r), moprand_process(d)]
        ...
        for i in range(mop.f.solid_args):
            mc_info.append(moprand_process(args.at(i)))
        if len(mc_info) == 0:
        ...
        hop = mop.pair.hop
        lop = mop.pair.lop
        oppair = [moprand_process(hop), moprand_process(lop)]
...
def get_mc(mba, mblock_num):
    ...
    for i in range(mblock_num - 2):
        ...
        while (cur):
            ...
            r = cur.r
            d = cur.d
            oriinst_mc_info = [opcode_str, [l.dstr(), moprand_list[l.t]], [r.dstr(), moprand_list[r.t]],
                               [d.dstr(), moprand_list[d.t]]]
            reguinst_mc_info = [opcode_str, moprand_process(l), moprand_process(r), moprand_process(d)]
    ...
    return ori_mc, regu_mc


def get_mc_graph_adjtable(graph):
    g = nx.DiGraph()
    adj_list = []
    if (graph.nsucc(0) > 1):
        print('yes')
...
def get_func_microcode_info(func, Opt_Level):
    ...
    try:
        ...
        ori_mc, regu_mc = get_mc(mba, mblock_num)
        # mc_graph_adjtable = get_mc_graph_adjtable(graph)#用不上图了
    except:
        return ori_mc, regu_mc, mc_graph_adjtable, mc_vars, nice_name
...
if __name__ == '__main__':
    ...
    savename = '-'.join(bin_name.split('-')[:-1])
    saving_dir = 'E:\\项目\\ida_analysis\\jtrans_json'
    # saving_path = saving_dir + '\\' + bin_name + '.json'
    if is_debug:
        saving_path = os.path.join(saving_dir, savename + '_debug.json')
    else:
        saving_path = os.path.join(saving_dir, savename + '.json')
    ...
    with open(saving_path, 'w') as save_f:
        ...
        for func in idautils.Functions(seg_sea, seg_eea):
            ...
            fdict['ori_mc'], fdict['regu_mc'], _, fdict['mc_vars'], fdict['nice_name'] = get_func_microcode_info(func,
                                                                                                                 MC_OPT)
